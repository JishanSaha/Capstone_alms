...
class AdminController extends Controller
{
    ...
    public function employee_leave_status_approve($id){
        // dd($id);
        $is_there = EmployeesLeave::find($id);
        if (is_null($is_there)) {
            return back()->with('error', "Something Went Wrong");
        }else{
                $is_there->update(['status'=> 1]);
    ...
    public function employee_leave_status_disapprove($id){
        // dd($id);
        $is_there = EmployeesLeave::find($id);
        if (is_null($is_there)) {
            return back()->with('error', "Something Went Wrong");
        }else{
                $is_there->update(['status'=> 2]);
                return back()->with('error', "Leave Disapproved Successfully");
        }
    }
}
